...
class EnvironmentLoop:
    ...
    def run_episode(self):
        ...
        for agent, value in episode_returns.items():
            logs[f"{agent}_episode_return"] = value
...
class EvaluationEnvironmentLoop:
    ...
    def run_evaluation(
        ...
        for episode in range(self._evaluation_episodes):
            ...
            for agent in self._environment._agents:
                agent_evaluation_episode_returns[agent].append(episode_returns[agent])
        ...
        evaluation_logs = {
            **{f"{agent}_evaluator_episode_return": np.mean(agent_evaluation_episode_returns[agent]) for agent in self._environment._agents},
