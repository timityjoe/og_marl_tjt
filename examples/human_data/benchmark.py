...
flags.DEFINE_string("env_name", "kaz", "E.g. 3m, 8m")
flags.DEFINE_string("dataset_quality", "", "E.g. Good, Medium, Poor")
flags.DEFINE_string("algo_name", "maicq", "E.g. qmix, qmix+cql, qmix+bcq, bc")
flags.DEFINE_string("max_trainer_steps", "20_001", "Max number of trainer steps.")
...
def build_qmix_bcq_system(num_agents, num_actions, environment_factory, logger_factory):
    system = QMIXBCQSystemBuilder(
        ...
        optimizer=snt.optimizers.Adam(3e-4),
        target_update_rate=0.01,
        threshold=0.6, # BC threshhold
        batch_size=64,
        add_agent_id_to_obs=True,
        max_trainer_steps=int(FLAGS.max_trainer_steps),
        must_checkpoint=True,
...
def build_qmix_cql_system(num_agents, num_actions, environment_factory, logger_factory):
    system = QMIXCQLSystemBuilder(
        ...
        optimizer=snt.optimizers.Adam(1e-5),
        target_update_rate=0.01,
        cql_weight=5.0,
        num_ood_actions=20,
        batch_size=32,
...
def main(_):
    ...
    # Environment factory
    environment_factory = functools.partial(KAZ, FLAGS.env_name)
